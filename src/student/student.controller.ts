...
import { LoginDto, RegisterDto } from "./student.validation";
...
export class StudentCourse {
  ...
  async Register(
    ...
    try {
      ...
      const register = await this.StudentService.Create({
        ...body,
        password: hashedPassword,
      });

      const payload: jwtPayload = { userId: register.id, loggedInAs: Roles.STUDENT }
  ...
  async Login(
    ...
    try {
      ...
      const one = await this.StudentService.One(email);
      ...
      await bcrypt.compare(password, one.password, function (err, result) {
        ...
        if (!result) {
          ...
          const payload: jwtPayload = { userId: one.id, loggedInAs: Roles.STUDENT }
          const token = jwt.sign(payload, "secret", { expiresIn: "4h" });
          console.log(one.id);
  ...
  async Change(
    ...
    try {
      ...
      if (!one) {
        ...
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt)
        await this.StudentService.Update({ password: hashedPassword }, email)
